...
class FinalViewProjects extends Component {
    ...
    ShowMissingJudge = (projectNumber, isMissing) => {
        ...
        const attribute = isMissing
            ? elem.setAttribute("title", "Le nombre de juge est inférieur ou supérieur au nombre de juges permis")
    ...
    ShowProjectInfos = e => {
        const target = document.querySelector(`.more-infos[data-project="${e.currentTarget.dataset.project}"]`);
                    ...
                    <Fragment>
                        ...
                        <div className="row border-bottom pb-2">
                            <div className="col-md-12 text-left">
                                {participant1 && (
                                    <span>
                                        {participant1.information.generalInformation.firstName} {participant1.information.generalInformation.lastName}
                                    </span>
                                )}
                                {participant2 && <span>&emsp;&amp;&emsp;</span>}
                                {participant2 && (
                                    <span>
                                        {participant2.information.generalInformation.firstName} {participant2.information.generalInformation.lastName}
                                    </span>
                                )}
                            </div>
                        </div>
    ...
    render() {
        ...
        const projectsList = projects.map(project => {
            const attributionByProject = isEmpty(final.pairing) ? {} : final.pairing.pairingByProjects[project.number];
                    ...
                    <div className="projectsListItem row" data-projectrow={project.number}>
                        ...
                        <div className="col-md-5 row">
                            <div className="col-1">
                                <strong>
                                    {project.number === null ? (
                                        <i className="fas fa-exclamation-triangle text-danger" title="Numéro non attribué" />
                                    ) : (
                                        project.number
                                    )}
                                </strong>
                            </div>
                            <div className="col">{project.information.projectInformation.title}</div>
                            <div className="col-1">
                                {this.FormatCategory(project.information.projectInformation.category, true)}
                                {"  "}
