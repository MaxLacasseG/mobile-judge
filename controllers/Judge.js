...
controller.Create = judgeInfos => {
    ...
    return Judge.find({ finalId: judgeInfos.finalId, judgeId: judgeInfos.judgeId }).then(result => {
        ...
        const pwd = `${firstName.substr(0, 1).toLowerCase()}${postalCode.substr(0, 3).toUpperCase()}${postalCode
            .substr(4, 3)
            .toUpperCase()}${lastName.substr(0, 1).toLowerCase()}${judgeInfos.specialCharacter}`;
