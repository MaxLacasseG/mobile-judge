...
const adminInitPwdValidator = require("../validators/AdminInitPwd");
...
router.put("/init-pwd", (req, res) => {
	const { isValid, errors, sanitizedData } = adminInitPwdValidator(req.body);
	if (!isValid) return res.status(400).json(errors);

	AdminController.InitPwd(sanitizedData)
		.then(resultat => {
			return res.status(200).json(resultat);
		})
		.catch(err => {
